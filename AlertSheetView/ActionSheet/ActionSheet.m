...
@property (nonatomic, copy) void(^click)(ActionSheetModel *model);
@end
...
+ (void)initialize
{
    ...
    dispatch_once(&onceToken, ^{
        sheet = [[ActionSheet alloc] initWithFrame:CGRectZero];
    });
}

+ (void)showWithDesc:(NSString *)desc actionModels:(NSArray<ActionSheetModel *> *)models action:(void (^)(ActionSheetModel *))action{
    sheet.desc = desc;
...
-(void)show{
    ...
    [UIView animateWithDuration:0.5 delay:0 usingSpringWithDamping:0.6 initialSpringVelocity:4 options:UIViewAnimationOptionCurveEaseOut animations:^{
...
-(void)createUI{
    ...
    for (int i = 0; i < self.actionModels.count; i++) {
        ActionSheetModel *model = self.actionModels[i];
        ...
        if (model.subTitle) {
            ...
            if (model.itemType == ActionSheetItemDelete) {
...
-(void)itemClick:(UIButton *)button{
    ...
    ActionSheetModel *model = self.actionModels[index];
